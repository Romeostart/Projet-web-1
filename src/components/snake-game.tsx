...
export function SnakeGame() {
  ...
  const changeDirection = useCallback((nextDirection: Direction) => {
    setQueuedDirection((currentDirection) =>
      opposite[currentDirection] === nextDirection ? currentDirection : nextDirection,
    );
  ...
  return (
    <div className="game-shell">
      ...
      <div className="game-board" role="application" aria-label="Jeu Snake">
        ...
        {status !== "playing" && (
          <div className="game-overlay">
            <p>{status === "over" ? "Partie terminée" : "Prêt à jouer"}</p>
            <Button onClick={status === "over" ? startNewGame : () => changeDirection(direction)} size="sm">
              {status === "over" ? "Rejouer" : "Commencer"}
            </Button>
          </div>
        )}
      </div>
      <div className="game-controls">
        <div className="direction-pad" aria-label="Commandes du jeu">
          <Button aria-label="Monter" onClick={() => changeDirection("up")} size="icon" variant="outline">
            <ArrowUp size={16} />
          </Button>
          <Button aria-label="Aller à gauche" onClick={() => changeDirection("left")} size="icon" variant="outline">
            <ArrowLeft size={16} />
          </Button>
          <Button aria-label="Descendre" onClick={() => changeDirection("down")} size="icon" variant="outline">
            <ArrowDown size={16} />
          </Button>
          <Button aria-label="Aller à droite" onClick={() => changeDirection("right")} size="icon" variant="outline">
            <ArrowRight size={16} />
          </Button>
        </div>
      ...
      <p className="game-hint">Touches fléchées ou WASD · Les bords se rejoignent · Attrape les carrés</p>
    </div>
  );
}
